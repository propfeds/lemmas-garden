...
const harvestFrame = createImageBtn({
    ...
    if (actionConfirm) {
        ...
        manager.performAction(plotIdx, slotIdx[plotIdx], 0 /* Actions.HARVEST */);
...
const pruneFrame = createImageBtn({
    ...
    if (actionConfirm) {
        ...
        manager.performAction(plotIdx, slotIdx[plotIdx], 1 /* Actions.PRUNE */);
}, () => true, game.settings.theme == Theme.LIGHT ?
        ...
        switch (colonyMode) {
            case 1 /* ColonyModes.VERBOSE */:
                ...
                c.energy, c.synthRate * BigNumber.from(insolationCoord), c.growth, c.stage < (plantData[c.id].maxStage ?? INT_MAX) ?
                    ...
                    BigNumber.ZERO, c.profit, slotIdx[plotIdx] + 1, manager.colonies[plotIdx].length, status)}}`;
                break;
            case 2 /* ColonyModes.SIMPLE */:
                ...
                \\text{p}\\\\(${slotIdx[plotIdx] + 1}/${manager.colonies[plotIdx].length})\\\\`;
                break;
