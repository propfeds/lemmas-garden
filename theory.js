...
Occasionally, visitors and artists, generous donors, they would come and ` +
            ...
            ginger: {
                title: 'Ginger',
                contents: `Brought back from lands far beyond the Twin Walls, where legends whispered ` +
...
** Ranges from 0 to 1. When omitted, the chance is assumed to be 1 (100%).`
            ...
            symbols: {
                ...
                contents: `A: apex (stem bud). Can photosynthesise.
 ...
 * @returns {string}
 */
let getSignedString = (x) => x >= BigNumber.ZERO ?
...
const broomrapeSpawner = {
    ...
    plot: (index) => gameRNG.nextInt % (Math.min(nofPlots, 1 + Math.floor(index / 3)))
};
...
class ColonyManager {
    ...
    importColony(destPlot, source, plot, index) {
        // Full!
...
var init = () => {
    ...
    */
    {
        shovelKill = theory.createPermanentUpgrade(9004, currency, new FreeCost);
    ...
    */
    {
        warpTick = theory.createPermanentUpgrade(9005, currency, new FreeCost);
        ...
        warpDay = theory.createPermanentUpgrade(9006, currency, new FreeCost);
        ...
        warpWeek = theory.createPermanentUpgrade(9007, currency, new FreeCost);
        ...
        warpYear = theory.createPermanentUpgrade(9008, currency, new FreeCost);
    ...
    */
    {
        warpZero = theory.createPermanentUpgrade(9009, currency, new FreeCost);
    ...
    perfs[7 /* Profilers.EQ_2 */].exec(() => {
        switch (colonyMode) {
            case 1 /* ColonyModes.VERBOSE */:
                ...
                result = `\\text{\\begin{array}{c}${getColonyTitleString(c, { colour: true })}\\\\${Localization.format(getLoc('colonyStats'), c.energy, 
                // @ts-expect-error
                getSignedString(c.synthRate * BigNumber.from(insolationCoord)), c.growth, c.stage < (plantData[c.id].maxStage ?? INT_MAX) ?
                    // @ts-expect-error
                    plantData[c.id].growthCost * BigNumber.from(c.sequence.length) :
                    '∞', c.stage < (plantData[c.id].maxStage ?? INT_MAX) ?
                    // @ts-expect-error
                    getSignedString(plantData[c.id].growthRate * BigNumber.from(growthCoord)) : '+0', c.profit, slotIdx + 1, manager.colonies[plotIdx].length, status)}\\end{array}}`;
                break;
...
let getExtraPotEquation = () => {
    ...
    switch (colonyMode) {
        case 1 /* ColonyModes.VERBOSE */:
            ...
            \\\\${Localization.format(getLoc('colonyStats'), c.energy, 
            // @ts-expect-error
            getSignedString(c.synthRate * BigNumber.from(insolationCoord)), c.growth, c.stage < (plantData[c.id].maxStage ?? INT_MAX) ?
                // @ts-expect-error
                plantData[c.id].growthCost * BigNumber.from(c.sequence.length) :
                '∞', c.stage < (plantData[c.id].maxStage ?? INT_MAX) ?
                // @ts-expect-error
                getSignedString(plantData[c.id].growthRate * BigNumber.from(growthCoord)) : '+0', c.profit, 1, 1, status)}\\end{array}}`;
            break;
        case 2 /* ColonyModes.SIMPLE */:
            // if(!isColonyVisible(c))
            // {
