...
var getDescription = (language: string): string =>
{
    ...
    {
        en:
`Last night, Léa had swept away the long-fallen leaves, the dead rats, and ` +
...
(submit to Tau by Friday)`
            ...
            prep:
            {
                title: 'Preparations!',
                contents:
`Before we start, let's decide on your seeds.
...
new ConstantCost(2), new CompositeCost(1,
new ConstantCost(1500), new CompositeCost(1,
new ConstantCost(44000),
new ConstantCost(270000))));
const permaCosts =
[
    BigNumber.from(9),
    BigNumber.from(180),
    ...
    */
    {
        ...
        getLoc('labelSpeed'), speeds[speedMs.level], Utils.getMath('\\times'));
